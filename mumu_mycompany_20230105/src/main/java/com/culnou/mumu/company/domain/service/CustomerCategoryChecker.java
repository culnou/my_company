...
@Service
@Transactional
public class CustomerCategoryChecker {
	...
	public String avarable(String customerCategoryId) throws Exception{
		String message = "OK";
		//プロジェクトやプログラムに割当てられている場合削除できない。
		List<ProjectDto> projects = projectService.findProjectsOfCustomerCategory(customerCategoryId);
		if(projects.size() > 0) {
			message = "already_assigned_to_projects";
		}
		List<ProgramDto> programs = programService.findProgramsOfCustomerCategory(customerCategoryId);
		if(programs.size() > 0) {
