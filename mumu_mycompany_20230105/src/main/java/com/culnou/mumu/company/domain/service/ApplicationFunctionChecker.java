...
@Service
@Transactional
public class ApplicationFunctionChecker {
	
	@Qualifier("actionJpaRepository")
	@Autowired
	private ActionRepository actionRepository;
	
	public String avarable(String applicationFunctionId) throws Exception{
		String message = "OK";
		List<Action> actions = actionRepository.findActionsOfApplicationFunction(applicationFunctionId);
		if(actions.size() > 0) {
