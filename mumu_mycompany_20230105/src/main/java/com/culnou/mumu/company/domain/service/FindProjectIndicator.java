...
@Service
@Transactional
public class FindProjectIndicator {
	...
	public List<Indicator> findIndicator(String projectId, IndicatorType indicatorType) throws Exception{
		List<Indicator> indicators = new ArrayList<>();
		if(projectId == null || projectId.isEmpty()) {
			throw new IllegalArgumentException("The_projectId_may_not_be_set_to_null");
		}
		ProjectDto project = projectService.findProjectDtoOfId(projectId);
		if(indicatorType.equals(IndicatorType.Business)) {
			BusinessUnit businessUnit = businessUnitRepository.businessUnitOfId(new BusinessUnitId(project.getBusinessUnitId()));
			...
			for(AssociatedCustomerCategory customerCategory : project.getAssociatedCustomerCategories()) {
			...
			for(AssociatedProductCategory productCategory : project.getAssociatedProductCategories()) {
			...
			for(AssociatedMemberCategory memberCategory : project.getAssociatedMemberCategories()) {
			...
			for(AssociatedPartnerCategory partnerCategory : project.getAssociatedPartnerCategories()) {
			...
			for(AssociatedPlaceCategory placeCategory : project.getAssociatedPlaceCategories()) {
			...
			for(AssociatedDataCategory dataCategory : project.getAssociatedDataCategories()) {
			...
			for(AssociatedApplicationCategory applicationCategory : project.getAssociatedApplicationCategories()) {
			...
			for(AssociatedFinancialAssetCategory financialAssetCategory : project.getAssociatedFinancialAssetCategories()) {
			...
			for(AssociatedActionPlan actionPlan : project.getAssociatedActionPlans()) {
